...
@menu_router.get("/new_game", response_model=GameDTO, status_code=status.HTTP_200_OK)
...
# Assuming generate_game_board returns a numpy array
@menu_router.get("/continue", response_model=GameDTO, status_code=status.HTTP_200_OK)
...
@menu_router.delete("/delete_game", response_model=UpdateMessageDTO, status_code=status.HTTP_200_OK)
...
async def leaderboard(db: db_dependency, limit: int = 5):
    ...
    return top_players if top_players else UpdateMessageDTO(detail="No players' scores found.")
